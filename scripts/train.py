...
class Trainer(object):
    ...
    def train(self):
        ...
        for iteration, (images, targets) in enumerate(self.train_loader):
            ...
            # visual_check(img_path, mask_path, outputs[0][0].cpu())
            ...
            if iteration % iter_per_epoch == 0:
                ...
                logger.info('Epoch: {0:03d} | Time: {4:.4f} | Avg. loss: {1:.4f} | pixAcc: {2:.4f} | Mean IoU: {3:.4f} | combined: {5:.4f}'
                            .format(epoch, training_loss, pixAcc, mIoU, epoch_duration, pixAcc + mIoU))
    ...
    def validation(self, epoch):
        ...
        logger.info('VALIDATION | Time: {3:.4f} | Avg. loss: {0:.4f} | pixAcc: {1:.4f} | Mean IoU: {2:.4f} | combined: {4:.4f}'
                    .format(avg_val_loss, pixAcc, mIoU, duration, pixAcc + mIoU))
